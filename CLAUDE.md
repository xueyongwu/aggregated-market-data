...
stock.crypto_perf     Kraken ───────────────────────────────────→ crypto_data.js → CryptoPage
...
python -m pipeline.stock.crypto_perf              # BTC/ETH 现货(独立 cron，见下)
...
python -m tests.test_crypto_perf         # Kraken 解析 + 前端换基准表(合成日线)
...
- **国债活跃券卡片（`BondPage`）**：`stock/bond_rate.py` 拉中国货币网现券成交（`chinamoney.com.cn/ags/ms/cm-u-md-bond/CbtPri`，akshare 的 `bond_spot_deal` 同源）→ 挑 10Y/30Y 活跃券 → `bond_data.js`，纯 HTML 卡片（无 ECharts）。约束：
  ...
  - 银行间 9:00-17:00 交易、20:00 定版，所以 `update.yml` 的两条 cron 里 **北京 18:00 那条才是准的**，16:30 那条写的是盘中快照（会被 18:00 覆盖）。`continue-on-error` 单独跑。改 `pick()` 跑 `tests/test_bond_rate.py`（期限解析 / 量最大 / 排除政金债与桶外 / 空收益率 / 空桶抛错 / 收益率出格六条分支，全离线）。
...
- **加密页（`CryptoPage` ← `crypto_data.js` ← `stock/crypto_perf.py`，独立 cron `crypto.yml`）**：BTC/ETH 美元现货，与美股三列同口径（复用 `us_perf` 的 `perf()`/`fetch()`/`last()`，不另起一套；`metrics()` 现在只是 `perf(closes(sym))` 的壳）。源是 **Kraken 公共 OHLC**（`api.kraken.com/0/public/OHLC?pair=XBTUSD&interval=1440`），一次 720 根日线 ≈2 年，三个基准都够，无 key 无 UA 要求。**响应键不等于请求的 pair**——传 `XBTUSD` 回的是 `XXBTZUSD`（Kraken 管 BTC 叫 XBT，ETH 是 `XETHZUSD`），只能挑 `result` 里除 `last` 外的那个键，别硬编码；行 `[ts,o,h,l,c,vwap,vol,count]` 数值全是字符串。其余源实测：**币安 451 地域封锁**（`api1` 同样，GH Actions 的美国 IP 一样挂，只有 `data-api.binance.vision` 通但那是镜像不保证长期）、OKX 本地通但美国 IP 有封锁风险故 CI 不能用、Coinbase 一次只给 350 根要 start/end 分页（可作备源，带 UA 否则空返回）、腾讯没有加密（`hf_BTC` 返回 `v_pv_none_match`）、新浪 `hf_BTC` 是 CME 比特币期货不是现货（有假日休市）。加密 7×24 按 UTC 日切，最新那根是当日进行中的 bar，所以 `close` 即实时价；周一必有 bar，「本周」基准天然落到上周日，`perf()` 不用开特例。**「今日/涨跌幅」列在 `perf()` 里（`day`）**，美股两张表和加密共用（`cut` = 最新那根本身 → 基准是前一根；加密 7×24 连续交易，昨日 UTC 收盘就是今日开盘，实测与 Kraken ticker 的 `o` 字段逐位相同）。拉取失败沿用上次 `crypto_data.js` 里的 `items` 并逐行标 `stale`（前端半透明 + 注明 + 不开实时），**从来没成功过则抛错不写文件**（这份文件只有加密，写个空的不如让 `continue-on-error` 兜着、页面留旧版）。改 `parse_kraken()`/`bases()` 跑 `tests/test_crypto_perf.py`。
  - **为什么从 `us_perf.py` 拆出来**（2026-08-08）：① 加密 7×24，`close` 每跑都变，挂在 `us_data.js` 里会让美股那份天天被顶着提交，「无变化就不提交」永远不成立（原来 CLAUDE.md 里那句「去掉 `updated` 也不成立」说的就是这个，现在成立了）；② `us.yml` 的 cron 是按美股收盘排的（UTC `1-5` = 北京周二~周六 07:00），周日周一不跑，浏览器拉不到实时价时降级快照最久陈旧 3 天。**拆分本身不解决基准窗口**，那是下一条的事。代价是 deploy 从 ~5 次/周涨到 ~12 次/周。
  - **前端 2s 轮询实时价**（`CryptoPage`）：**Kraken 开了 CORS**（回显 Origin，Coinbase/CoinGecko 是 `*`），浏览器直接 `fetch('/0/public/Ticker?pair=XBTUSD,ETHUSD')` 即可，不用 `QdiiPage` 那套 `<script>` 注入绕；`result.<key>.c[0]` 是最新成交价，**键同样被改名**，按币种前三位（`XBT`/`ETH`）反查而不是信对象键顺序。
  - **基准由前端按 UTC 时钟现查，不再吃 CI 烤死的那个数**（`baseDates()` + `bases()`，2026-08-08 起）。原来 `perf()` 把「基准 → 涨跌幅」算死成一个数字，前端只换分子（`(1+p/100) × 实时价 / 那根收盘 − 1`），而四个基准全都随 **UTC 日切**移动 —— 于是「UTC 边界 → 下一次 CI」之间页面用的是上一档基准，**周切那个窗口曾长达 23 小时**（UTC 周切 = 北京周一 08:00，下次 CI 是周二 07:00）。现在 `crypto_perf.bases()` 把基准收盘一起导出成 `{"YYYY-MM-DD": close}`，前端从此刻的 UTC 时钟推出四个基准日期查表，**窗口是 0，cron 什么时候跑都不影响正确性**。要点：
    - **带哪几根**：最近 10 根完整日线 + 当前 mtd/ytd 那两根。日切/周切时新基准最远是上周日（7 天前，在 10 根里）；月切/年切时新基准就是「昨天」（上月最后一天 / 12-31，也在 10 根里）；不切的日子 mtd 可能 30 天前、ytd 可能 365 天前，所以单独带。**今天那根必须按 UTC 日期剔掉**（它是进行中的 bar，`close` 是实时价不是收盘，混进去前端会把「今日」基准查成今天自己）——`bases(s, today)` 收 `today` 参数就是为了这个可测。
    - **只在拿到实时价时换基准**。没实时价时 `c.close` 可能已是昨天那根，拿它算「今日」会得出昨天的日涨跌却标成今天，不如原样显示 CI 快照 + 副标题的「截至 …（UTC）」。查不到基准（CI 连挂 >7 天）才退回只换分子的老算法。`stale` 时不开实时（基准本身就旧）。
    - JS 侧用 `Date.UTC(y, m, 0)` = 上月最后一天、`Date.UTC(y, 0, 0)` = 去年 12-31，不用自己判闰年；周基准是 `d - ((getUTCDay()+6)%7) - 1`（本周一再退一天 = 上周日）。仓库没有 JS 测试框架，两侧的对齐靠 `tests/test_crypto_perf.py` 里写死的那几个日期字面量兜（2026-08-08 → day 08-07 / wtd 08-02 / mtd 07-31 / ytd 2025-12-31），改任一侧都要对着另一侧核。
    - 残留缺口：**依赖浏览器本地时钟**（时区无所谓，用的全是 UTC 字段；但系统时间差出一整天会查错基准）。原来的 CI 口径没这个问题，属明知的取舍。
