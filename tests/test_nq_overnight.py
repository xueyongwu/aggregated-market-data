...
def test_complete_point():
    ...
    it = items[0]
    assert (it["d"], it["base"], it["pct"]) == ("2026-07-21", 100.0, 1.0), it
...
class FakeResp:
    """新浪正常响应: jsonp 包一层, minLine_1d 每行尾部 [-5]=价 [-1]=时间戳。"""
    text = ('var t=({"minLine_1d":[["2026-07-21","0","CME","",'
            '"100.0","0","0","0","2026-07-21 06:00:00"]]});')

    def raise_for_status(self): pass


def fake_get(fails):
    """前 fails 次抛错, 之后返回正常响应; 顺带记录实际调用次数。"""
    calls = []
...
def patched(get, fn):
    """替掉 requests.get 与 sleep(免得测试真等), 跑完还原。"""
    old_get, old_sleep = nq_overnight.requests.get, nq_overnight.time.sleep
    nq_overnight.requests.get, nq_overnight.time.sleep = get, lambda s: None
    try:
        return fn()
    finally:
        nq_overnight.requests.get, nq_overnight.time.sleep = old_get, old_sleep


def test_fetch_retries_then_succeeds():
    """前两次失败第三次成功 -> 正常出 bar, 不抛。"""
    get, calls = fake_get(fails=2)
    df = patched(get, fetch_bars)
    assert len(calls) == 3, calls
    assert df["price"].tolist() == [100.0], df
    assert df["dt"].tolist() == [pd.Timestamp("2026-07-21 06:00")], df


def test_fetch_raises_after_all_tries():
    """次次失败 -> 试满 3 次后抛出(CI 该步红掉, 不静默写空)。"""
    get, calls = fake_get(fails=99)
    try:
        patched(get, fetch_bars)
    except RuntimeError:
        assert len(calls) == 3, calls
    else:
        raise AssertionError("试满仍应抛错")
