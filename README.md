...
四块市场数据看板合并成的一个单页应用，数据全部由 GitHub Actions 定时抓取后自动部署到 GitHub Pages。

| 页面 | 路由 | 内容 |
| --- | --- | --- |
| A股中位数 | `#/a` | 全市场当日涨跌幅中位数、累计中位数、涨跌停分布、10Y/30Y 国债活跃券收益率、13 个宽基指数 YTD 排名 |
| 纳指QDII | `#/etf` | 纳指期货隔夜涨跌 + 分时、12 只场内纳指 QDII 的溢价/区间涨跌对照、159696 日K + 分时 |
| 美股 | `#/us` | 纳指100 与七巨头的周/月/年涨跌、BTC/ETH 现货、QQQ 前 10 大权重股（含最新财报）、行业权重 |
| 70城房价 | `#/housing` | 国家统计局 70 城新建商品住宅/二手住宅环比指数，2021 年至今 |

多个卡片带浏览器端实时轮询（国债 60s、场内快照 8s、加密 2s），CI 那份只是开页时的静态基准。
深浅色主题全站统一，选择记在 `localStorage`。
